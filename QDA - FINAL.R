...
for (i in 1:100){ # i measures the number of times we will update optimal.accuracy.rate.
  ...
  for (j in 1:ncol(df)){ # this creates all possible permutations of the columns
    ...
    for (k in 1:nrow(combin)){ # Loop to create all possible data sets by column, then we can do the analysis of each one
      ...
      qda.fit <- qda(last~.,data = train.df)
      qda.pred <- predict(object = qda.fit,newdata = test.df)
      qda.class <- qda.pred$class
      table1 <- table(qda.class, last[-train])
      ...
      #### - Measure overall accuracy of the qda ####
      if(nrow(table1) == ncol(table1)){
        ...
        if (accuracy>accuracytmp){
          ...
          saveRDS(qda.fit, file = "/tmp/qda.max.accuracy.rda")
        }
      }
      ...
      if (nrow(table1) == ncol(table1)){
        ...
        if(sensitivity>sensitivitytmp | sensitivity == 1){
          if(table1[4:4]> temptable[4:4]){
            ...
            temptable = table1
            saveRDS(qda.fit, file = "/tmp/qda.max.sensitivity.rda")
          }
        }
      }
      ...
      if (nrow(table1) == ncol(table1)){
        ...
        if(specificity>specificitytmp | specificity == 1){
          if(table1[1:1]> temptable1[1:1]){
            ...
            temptable1 = table1
            saveRDS(qda.fit, file = "/tmp/qda.max.specificity.rda")
          }
        }
      }   
