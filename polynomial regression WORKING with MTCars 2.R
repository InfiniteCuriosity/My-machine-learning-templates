...
i = 1
last <- Carseats$Price # This is our target, we will use this to measure the success of our analysis
...
for (i in 1:10){
  ...
  for (j in 1:ncol(df)){ # this creates all possible permutations of the columns
    ...
    for (k in 1:nrow(combin)){
      ...
      print(sprintf("Min Mean Squared Error train %f", mse.train))
      print(sprintf("Min Mean Squared Error test %f", mse.test))
      ...
      train.df <- newdf[idx == 1,]
      test.df <- newdf[idx == 2,]
      validation.df <- newdf[idx == 3,]
      ...
      if (((Mean.Squared.Error < min(min.MSE, mse.train)) | (Mean.Squared.Error1 < min(min.MSE, mse.test)) | (Mean.Squared.Error2 < min(min.MSE, mse.validation)))){
...
best.model <- c("Train", "Test", "Validation", "Duration")
max.adjusted.r.squared <- c(adj.rsquared1,adj.rsquared2,adj.rsquared3, duration)
best.min.mse <- c(mse.train, mse.test, mse.validation,0)
