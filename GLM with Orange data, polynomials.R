...
# df <- diamonds
# df$value <- ifelse(df$price>3000,1,0)
# df <- dplyr::select(df,carat, cut, color, clarity, depth, table,x,y, z, value)
...
df$Purchase <- ifelse(df$Purchase == "CH", 1, 0)
df <-  df %>% dplyr::relocate(Purchase, .after = last_col())
...
for (i in 1:10){ # i measures the number of times we will update optimal.accuracy.rate.
  ...
  print(i)
  for (j in (ncol(df)-5):ncol(df)){ # this creates all possible permutations of the columns
    ...
    for (k in 1:nrow(combin)){ # Loop to create all possible data sets by column, then we can do the analysis of each one
      cat("\014")
      print(sprintf("Iteration %.01f of %.01f", i, 100))
      print(sprintf("Column %.01f of %.01f", j, ncol(df)))
      print(sprintf("%s", colnames(df)[colvals]))
      print(sprintf("Row %.01f of %.01f", k, nrow(combin)))
      print(sprintf("The maximum accuracy is %f", accuracytmp))
      colvals <- c(combin[k,])
...
#        newdf <- newdf^(l)
      ...
      glm.fits1 <- glm(last~., data = df, family = binomial)
      glm.probs <- predict(glm.fits1, df, type = "response")
      glm.pred <- rep("No", nrow(test.df))
      glm.pred[glm.probs > optimal.accuracy1.rate] = "Yes"
      table1 <- table(glm.pred, last)
      accuracy.train <- sum(diag(table1))/sum(table1)
      
      prediction <- prediction(predictions = glm.probs,labels = last)
      ...
      # prediction <- prediction(predictions = glm.probs2,labels = train.df$last)
      # cost_perf = performance(prediction, "cost")
      ...
      if(accuracy.train>accuracytmp){
        if(nrow(table1) == ncol(table1)){
          if(ncol(table1) == 2){
            #            tables.df <- rbind(tables.df, table1.df)
#            if(abs(accuracy.train-accuracy.test) < diff){
            accuracytmp <- accuracy.train
