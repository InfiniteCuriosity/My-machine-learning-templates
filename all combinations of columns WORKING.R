...
for(i in 1:10){
  for(j in 1:99){
      for (k in 1:ncol(df)){ # this creates all possible permutations of the columns
        combin <- combinations(n = ncol(df), r = k, repeats.allowed = FALSE)  #### change back to a variable!!!
        for (l in 1:nrow(combin)){
          colvals <- c(combin[l,]) ##### change back to a variable!!
          ...
          names(df)[names(train.df)==names(df[ncol(df)])] = 'last'
          test.df <- newdf[-train,]
          names(df)[names(test.df)==names(df[ncol(df)])] = 'last'
