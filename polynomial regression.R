  ...
  for (j in 1:ncol(df)){ # this creates all possible permutations of the columns
    ...
    for (k in 1:nrow(combin)){
      ...
      for (l in 1:100) {
        ...
        m = m+1
        cat("\014")
        print(sprintf("Iteration %0.1f of 1", i))
        ...
        newdf.lm = lm(last~poly(df[train,j],2), data = train.df)
        ...
        newdf.lm1 = lm(last~poly(df[-train,j],2), data = test.df)
        ...
        if (abs(adj.r.squared.train - adj.r.squared.test) < 0.01){
          if(adj.r.squared.test > max.adj.r.squared){
            ...
            info1 <- (sprintf("j = %s", colnames(df)[j]))
          }
        }              
        if (Mean.Squared.Error & Mean.Squared.Error1 < min.MSE){
          ...
          info2 <- (sprintf("j = %s", colnames(df)[j]))
        }
      }
    }
  }
